...
async def extract_and_add_tasks(
    ...
    """Extract tasks and add them as cards to a Trello list. If the list or board doesn't exist, they will be created."""
    ...
    try:
        ...
        if not board:
            board = client.add_board(board_name)  # Create new board if not found

        # Step 3: Find or create the list
        lists = board.list_lists()
        target_list = next((l for l in lists if l.name.lower() == list_name.lower()), None)

        if not target_list:
            target_list = board.add_list(name=list_name, pos="bottom")  # Create list
